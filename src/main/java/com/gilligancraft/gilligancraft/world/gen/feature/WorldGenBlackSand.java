...
public class WorldGenBlackSand extends WorldGenerator
{
    ...
    public WorldGenBlackSand(Block blockType, int radius, boolean allowIncorrectDecorations, boolean cleanIncorrectDecorations)
    {
    ...
    // If it is too slow, the user config (TODO) can allow a sloppier implementation
    public WorldGenBlackSand(Block blockType, int radius)
    {
    	this(blockType, radius, false, true);
    }
                ...
                for (int j1 = z - l; j1 <= z + l; ++j1)
                {
                    ...
                    if (k1 * k1 + l1 * l1 <= l * l)
                    {
                        for (int i2 = y - b0; i2 <= y + b0; ++i2)
                        {
                            ...
                            {
                            	// If we're okay with incorrect decorations (like vanilla does), such as
                            	// grass on sand (which works during decoration but is invalid afterwards)
                            	...
                            	if (this.allowIncorrectDecorations) {
                            		...
                            		if (this.cleanIncorrectDecorations) {
                                    	...
                                    	if (block instanceof IPlantable) {
                                    		...
                                    		System.out.println("[" + i1 + "," + i2 + 1 + "," + j1 + "] : " + block.getUnlocalizedName() + " --> air");
                                    	}
                            		}
                            		else {
                            			
                            		// We don't care about invalid generation (a la vanilla MC)
		                        	// Only modification here is the last flag, which is 3 instead of 2
		                        	// 2 = Send change to Client, 1 = Update Block;  We want both, so set flag = 3
		                        	// We don't want random decorations (grass, flowers, etc.) on this block,
                            		// but we're not going to clean them up.  This will simply pop the decoration
                            		// and leave a floating item.
